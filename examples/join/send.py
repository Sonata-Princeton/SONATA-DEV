...
def send_traffic(dst):
    ...
    total_pkts = 0
    tmp_ports = range(0, 70)
    dst_ports = [x + 10000 for x in tmp_ports]
    observe_dst_ports = [10000, 10060, 10050, 10040, 10030, 10020, 10010]

# Send some packets for the join filter
    for port in observe_dst_ports:
        data = randomword(100)
        p = Ether(dst=dst_mac, src=src_mac)/IP(dst=dst_ip, src=src_ip)
        p = p/TCP(sport=9999, dport=port)/Raw(load=data)
        print p.show()
        sendp(p, iface="eth0")
        total_pkts += 1

    for port in dst_ports:
        num_packets = random.randint(1, 30)
