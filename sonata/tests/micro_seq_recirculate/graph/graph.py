...
def plot_line3(xlab, data, xlab2, data2, xlab3, data3, XLabel, YLabel, fname):

    fig = plt.figure()
    ax = fig.add_subplot(1,1,1)

    pl.plot(range(xlab), data, color=color_n[0],linestyle=linestyles[0], linewidth=2.0, label='recirulate')
    pl.plot(range(xlab2), data2, color=color_n[1],linestyle=linestyles[1], linewidth=2.0, label='sequential')
    pl.plot(range(xlab3), data3, color=color_n[2],linestyle=linestyles[2], linewidth=2.0, label='traffic')
...
f = open('../recirculate/emitter.log')
csv_f = csv.reader(f)
...
for ts in ordered_ts:
    recirculate.append(ts_packet_count[ts])

recirculate = [0, 0, 0, 0] + recirculate


f = open('../sequential/emitter.log')
csv_f = csv.reader(f)

ts_packet_sequential = {}

for row in csv_f:
    print row
    (name, qid, start, end) = row
    ts = round(float(start), 0)
    if ts not in ts_packet_sequential:
        ts_packet_sequential[ts] = 0
    ts_packet_sequential[ts] += 1

print "Number of TSs: ",len(ts_packet_sequential.keys())
print ts_packet_sequential

sequential = []
ordered_ts_seq = ts_packet_sequential.keys()
ordered_ts_seq.sort()
print ordered_ts_seq
for ts in ordered_ts_seq:
    sequential.append(ts_packet_sequential[ts])

sequential = [0, 0, 0, 0] + sequential
...
print recirculate, len(ordered_ts)
plot_line3(len(recirculate), recirculate,
           len(sequential), sequential,
           len(data_original[1:]), data_original[1:],
          'Time (seconds)',
          'Number of Packets',
          'macro_n_packets')
