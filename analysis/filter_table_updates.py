                ...
                for b_max in Bs:
                    ...
                    for ts in timestamps[td:]:
                        ...
                        for r,p,l in v:
                            if p > p_max:
                                p_max = p
                        data_dump[mode][(n_max, b_max)][ts] = 0
                        for (e1, e2) in zip(trained_path[:-1], trained_path[1:])[:-2]:
                            r2, p2, l2 = e2.state
                            e2_new = r2, p_max, l2

                            # print e1, e2, e[(e1.state, e2_new)]
                            data_dump[mode][(n_max, b_max)][ts] += e[(e1.state, e2_new)][1]

        print data_dump
        fname_dump = 'data/updates_' + str(datetime.datetime.fromtimestamp(time.time())) + '.pickle'
